...
define(['stylus'], () => {
  ...
  fetchText = ((() => {
    switch (env) {
      case 'node':
        return (path, cb) => cb(fs.readFileSync(path, 'utf8'));
      case 'browser':
        getXhr = () => {
          var e;
          var progId;
  ...
  return {
    ...
    load(name, parentRequire, load, config) {
      ...
      return fetchText(path, data => {
        data = data.replace(/\t/g, "  ").replace(/^\s+/, "").replace(/\s+$/, "");
        return stylusCompiler(data).render((err, css) => {
          var cssInsertString;
          ...
          return parentRequire([name], value => load(value));
        });
      });
    },
